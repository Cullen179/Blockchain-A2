...
export class Transaction {
  ...
  static async verifyTransaction(transaction: ITransaction): Promise<{
    success: boolean;
    change: number;
  }> {
    // Verify sender and recipient addresses
    const senderWallet = await WalletRepository.findByAddress(transaction.from);
    if (!senderWallet) {
      console.error(`Sender wallet not found for address: ${transaction.from}`);
      throw new Error(
    ...
    const areInputsValid = await this.validateTransactionInputs(transaction);
   ...
   */
  static async validateTransactionInputs(transaction: ITransaction): Promise<{
    success: boolean;
    change: number;
  }> {
