...
const mockTransactions: ITransaction[] = [
  {
    ...
    inputs: [{ previousTransactionId: 'genesis-utxo-1', outputIndex: 0, scriptSig: 'alice-signature' }],
    outputs: [{ amount: 97.5, address: 'bob-wallet-5678', scriptPubKey: 'pubkey-bob' }],
    size: 250
  },
  {
    ...
    inputs: [{ previousTransactionId: 'genesis-utxo-2', outputIndex: 1, scriptSig: 'charlie-signature' }],
    outputs: [{ amount: 49, address: 'alice-wallet-1234', scriptPubKey: 'pubkey-alice' }],
    size: 240
  }
];
...
mockTransactions.forEach((tx, i) => {
  console.log(`   TX${i+1}: ${tx.from.substring(0,10)}... → ${tx.to.substring(0,10)}... (${tx.amount} BTC)`);
});
...
const mockBlockData = {
  ...
  previousHash: '0000abc123def456789fedcba0987654321123456789012345678901234abcd',
...
console.log(`   Previous Hash: ${mockBlockData.previousHash.substring(0, 20)}...`);
...
console.log(`   Verification Result: ${originalVerification.isValid ? '✅ VALID' : '❌ INVALID'}`);
...
console.log(`   Verification: ${nonceVerification.isValid ? '❌ STILL VALID (BAD!)' : '✅ INVALID (GOOD!)'}`);
...
console.log(`   Original Prev Hash: ${completeBlock.previousHash.substring(0, 20)}...`);
console.log(`   Altered Prev Hash: ${alteredPrevHashBlock.previousHash.substring(0, 20)}...`);
console.log(`   Verification: ${prevHashVerification.isValid ? '❌ STILL VALID (BAD!)' : '✅ INVALID (GOOD!)'}`);
...
console.log(`   Original TX Amount: ${completeBlock.transactions[0].amount} BTC`);
console.log(`   Altered TX Amount: ${alteredTxBlock.transactions[0].amount} BTC`);
console.log(`   Verification: ${txVerification.isValid ? '❌ STILL VALID (BAD!)' : '✅ INVALID (GOOD!)'}`);
...
console.log(`   Verification: ${invalidHashVerification.isValid ? '❌ STILL VALID (BAD!)' : '✅ INVALID (GOOD!)'}`);

console.log('\n=== Hash Verification Summary ===');
console.log(`✅ Original Block: ${originalVerification.isValid ? 'VALID' : 'INVALID'}`);
console.log(`❌ Altered Nonce: ${nonceVerification.isValid ? 'VALID (PROBLEM!)' : 'INVALID (CORRECT)'}`);
console.log(`❌ Altered Previous Hash: ${prevHashVerification.isValid ? 'VALID (PROBLEM!)' : 'INVALID (CORRECT)'}`);
console.log(`❌ Altered Transaction: ${txVerification.isValid ? 'VALID (PROBLEM!)' : 'INVALID (CORRECT)'}`);
console.log(`❌ Invalid Hash Format: ${invalidHashVerification.isValid ? 'VALID (PROBLEM!)' : 'INVALID (CORRECT)'}`);
