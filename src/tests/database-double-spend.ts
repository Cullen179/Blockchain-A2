...
const generateAddress = (name: string): string => {
  return crypto
    .createHash('sha256')
    .update(`test-wallet-${name}-${Date.now()}`)
    .digest('hex');
};
...
async function createTestWallet(address: string, name: string): Promise<void> {
  console.log(
    `👤 Creating test wallet for ${name}: ${address.substring(0, 16)}...`
  );
  ...
  try {
    ...
    console.log(
      `   ❌ Failed to create wallet for ${name}: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    throw error;
  }
}
...
async function createTestTransactionWithUTXO(
  transactionId: string,
  from: string,
  to: string,
  amount: number,
  fee: number
): Promise<{ transaction: ITransaction; utxo: IUTXO }> {
  console.log(
    `💾 Creating transaction and UTXO in database: ${transactionId.substring(0, 20)}... → ${to.substring(0, 16)}... (${amount} BTC)`
  );
  ...
  try {
    ...
    const transaction: ITransaction = {
      ...
      size: Buffer.byteLength(
        JSON.stringify({ from, to, amount, fee }),
        'utf8'
      ),
    };
    ...
    console.log(
      `   ❌ Failed to create transaction/UTXO: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    throw error;
  }
}
...
async function checkUTXOStatus(
  ...
  try {
    ...
    if (utxo) {
      ...
      console.log(
        `   📊 UTXO Status: ${utxo.transactionId.substring(0, 20)}... → ${status} (${utxo.amount} BTC)`
      );
    } else {
    ...
    console.log(
      `   ❌ Error checking UTXO: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    return null;
  }
}
...
async function demonstrateDatabaseDoubleSpend() {
  try {
    ...
    const { transaction: tx1, utxo: utxo1 } =
      await createTestTransactionWithUTXO(
        'genesis-block-reward-001',
        'system',
        aliceAddr,
        100,
        0
      );
    const { transaction: tx2, utxo: utxo2 } =
      await createTestTransactionWithUTXO(
        'mining-reward-alice-002',
        'mining-pool',
        aliceAddr,
        50,
        0
      );
    const { transaction: tx3, utxo: utxo3 } =
      await createTestTransactionWithUTXO(
        'previous-transaction-003',
        'previous-sender',
        aliceAddr,
        25,
        1
    ...
    console.log(
      `     Using UTXO: ${utxo1.transactionId.substring(0, 20)}... (${utxo1.amount} BTC)`
    );
    console.log(
      `     Change: ${utxo1.amount - validTx.amount - validTx.fee} BTC`
    );

    // Check UTXO availability before processing
    console.log(`   🔍 Checking UTXO availability...`);
    const utxoBeforeSpend = await checkUTXOStatus(
      utxo1.transactionId,
      utxo1.outputIndex
    );
    ...
    if (utxoBeforeSpend && !utxoBeforeSpend.isSpent) {
      ...
      // Mark UTXO as spent
      const markResult = await UTXORepository.markAsSpent(
        utxo1.transactionId,
        utxo1.outputIndex
      );
      if (markResult) {
        console.log(
          `   ✅ Transaction 1 PROCESSED - UTXO marked as spent in database`
        );
    ...
    const doubleSpendTx = createMockTransaction(
      aliceAddr,
      charlieAddr,
      40,
      2,
    ...
    console.log(
      `     Using SAME UTXO: ${utxo1.transactionId.substring(0, 20)}... (${utxo1.amount} BTC)`
    );

    // Check UTXO availability (should fail)
    console.log(`   🔍 Checking UTXO availability for double spend...`);
    const utxoForDoubleSpend = await checkUTXOStatus(
      utxo1.transactionId,
      utxo1.outputIndex
    );
    ...
    if (utxoForDoubleSpend && !utxoForDoubleSpend.isSpent) {
      console.log(
        `   ❌ ERROR: UTXO appears available - this should not happen!`
      );

      // This would mark it as spent again (which shouldn't be possible)
      const doubleSpendResult = await UTXORepository.markAsSpent(
        utxo1.transactionId,
        utxo1.outputIndex
      );
    ...
    const validTx2 = createMockTransaction(
      aliceAddr,
      charlieAddr,
      20,
      1,
    ...
    console.log(
      `     Using Different UTXO: ${utxo2.transactionId.substring(0, 20)}... (${utxo2.amount} BTC)`
    );
    console.log(
      `     Change: ${utxo2.amount - validTx2.amount - validTx2.fee} BTC`
    );

    // Check UTXO availability
    console.log(`   🔍 Checking different UTXO availability...`);
    const utxo2Status = await checkUTXOStatus(
      utxo2.transactionId,
      utxo2.outputIndex
    );
    ...
    if (utxo2Status && !utxo2Status.isSpent) {
      console.log(
        `   ✅ Different UTXO is available, processing transaction...`
      );

      // Process the transaction
      const markResult2 = await UTXORepository.markAsSpent(
        utxo2.transactionId,
        utxo2.outputIndex
      );
      if (markResult2) {
        console.log(
          `   ✅ Transaction 2 PROCESSED - Different UTXO marked as spent`
        );
    ...
    const finalUTXOCheck = await checkUTXOStatus(
      utxo1.transactionId,
      utxo1.outputIndex
    );
    ...
    try {
      ...
      unspentUTXOs.forEach((utxo, index) => {
        console.log(
          `     UTXO ${index + 1}: ${utxo.amount} BTC from ${utxo.transactionId.substring(0, 20)}...`
        );
      });

      const totalBalance =
        await UTXORepository.getTotalValueByAddress(aliceAddr);
      console.log(`   📈 Total Unspent Balance for Alice: ${totalBalance} BTC`);
    } catch (error) {
      console.log(
        `   ❌ Error querying unspent UTXOs: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
...
demonstrateDatabaseDoubleSpend()
  .then(() => {
    console.log('\n=== Database Double Spend Prevention Summary ===');
    console.log('🔐 Key Security Features Demonstrated:');
    console.log(
      '✅ 1. UTXO Existence Validation: Check database before spending'
    );
    ...
    console.log(
      '✅ 5. Balance Calculation: Query unspent UTXOs for wallet balance'
    );
  })
  .catch(error => {
    console.error(
